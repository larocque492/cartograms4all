  ...
  d3.cartogram = function() {
    ...
    function carto(topology, geometries) {
      ...
      while (i2 < len2) { //iterates through the length of the arcs array
        ...
        out1 = new Array(len1);
        while (i1 < len1) {
          topo.arcs[i2][i1][0] = (x += topo.arcs[i2][i1][0]);
          topo.arcs[i2][i1][1] = (y += topo.arcs[i2][i1][1]);
          out1[i1] = projection(tf(topo.arcs[i2][i1]));
          i1++;
        }
      ...
      var objects = object(projectedArcs, {
        ...
        .geometries.map(function(geom) {
          return {
            type: "Feature",
            id: geom.id,
            properties: properties.call(null, geom, topo),
            geometry: geom
          };
        });
      ...
      while (i++ < iterations) {
        ...
        var len1, i1, delta, len2 = projectedArcs.length,
          i2 = 0,
          delta, len3, i3, centroid, mass, radius, rSquared, dx, dy, distSquared, dist, Fij;
        while (i2 < len2) {
