/**
 * Created by timotongo on 6/15/17.
 */
/**
...
(function(exports) {
    ...
    d3.cartogram = function() {
        ...
        function carto(topology, geometries) {
            ...
            }//the position arrays x and y have not converged


            function CALCV(t){
                //calculate density rho by filling the array with Fourier coefficients
                for (j=0; j===LX; j++){
                    for (k=0; k===LY; k++){
                        rho[j][k] = function(j,k,LX,LY) {

                        };
                    }
                }
                //calculate the fourier coefficients for the partial derivative of rho.
                //store temporary result in arrays gridvx, gridvy
                for (j=0; j===LX; j++) {
                    for (k=0; k===LY; k++) {
                        gridvx[j][k]= function(j,k,LX){
                            return -pi*j/LX*rho[j][k];
                        };
                        gridvy[j][k]= function(j,k,LY){
                            return -pi*j/LY*rho[j][k];
                        };
                    }
                }
                //replace rho by cosine Fourier backtransform in both variables
                rho[j][k]=ifft(rho[j][k]);

                //replace vx by sine Fourier backtransform in the first variable
                // and cosine in the second
                //...

                //replace vy by cosine Fourier backtransform in the first
                // sine in the second
                //...
